...
resource "frstudentservices_vpc" "primary_vpc" {
  name       = "Primary VPC"
  cidr_block = "0.0.0.0/1"
}

resource "frstudentservices_server" "servers" {
  count = 2

  name = "Server ${count.index + 1}"
  type = "t2.micro"
  vpc  = frstudentservices_vpc.primary_vpc.name
}

resource "frstudentservices_load_balancer" "primary_lb" {
  name    = "Primary Load Balancer"
  servers = frstudentservices_server.servers[*].name
}

resource "frstudentservices_database" "prod_db" {
  name = "Production DB"
  size = 256
}
