...
В большой системе можут сочетаться принципы BASE и ACID. Во всяком случае это могут быть комоненты
какой-то инфраструктуры. Насколько я понял из материала, эти принципы взаимоисключающие, поэтому в пределах одного компонента такое
сочетание маловероятно.
