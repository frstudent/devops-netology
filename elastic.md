...
Такой подход имеет несколько преимуещств - при сбое в контейнере данные не потеряются, контейнер легко переносим, упрощается бэкап данных.
...
Запускаем вышеприведённый скрипт и убеждаемся что elasticsearch удачно стартовал с помощью следующего скрипта.
...
```
По моим наблюдениям самая актуальная причина деградации скорости elasticsearch это нехватка памяти. Java очень прожорлива. Когдя я присутпил к решению задачи, 
...
```
Замечу, что эту операцию пришлось делат два раза, первый раз на хосте, ещё до разворачивания в контейнере, второй раз в контейнере.
